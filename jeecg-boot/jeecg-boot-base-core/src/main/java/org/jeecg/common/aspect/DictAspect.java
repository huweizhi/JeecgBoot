...
//    }
    ...
    private Object parseDictText(Object result) {
        ...
        if (((Result) result).getResult() instanceof IPage) {
            ...
            //update-end--Author:zyf -- Date:20220606 ----for：【VUEN-1230】 判断是否含有字典注解,没有注解返回-----
            for (Object record : records) {
                items.add(parseSingleObjectDictText(record, dictFieldList, dataListMap));
            }
            ...
            //step.3 将翻译结果填充到返回结果里
            for (JSONObject record : items) {
                this.translateDictText(record, dictFieldList, translText);
            }
            ...
            if (!this.checkHasDict(record)) {
                return result;
            }
            JSONObject item = this.parseSingleObjectDictText(record, dictFieldList, dataListMap);
            //step.2 调用翻译方法，一次性翻译
            Map<String, List<DictModel>> translText = this.translateAllDict(dataListMap);
            this.translateDictText(item, dictFieldList, translText);
    ...
    private JSONObject parseSingleObjectDictText(Object record, List<Field> dictFieldList, Map<String, List<String>> dataListMap) {
        String json = "{}";
        try {
            //update-begin--Author:zyf -- Date:20220531 ----for：【issues/#3629】 DictAspect Jackson序列化报错-----
            //解决@JsonFormat注解解析不了的问题详见SysAnnouncement类的@JsonFormat
    ...
    private void translateDictText(JSONObject record, List<Field> dictFieldList, Map<String, List<DictModel>> translText) {
        for (Field field : dictFieldList) {
            String code = field.getAnnotation(Dict.class).dicCode();
            String text = field.getAnnotation(Dict.class).dicText();
            String table = field.getAnnotation(Dict.class).dictTable();
    ...
    private Boolean checkHasDict(List<Object> records) {
        ...
        return this.checkHasDict(records.get(0));
    }
     ...
     * @return
     */
    private Boolean checkHasDict(Object record) {
        if (record == null) {
            return false;
        }
        return Arrays.stream(oConvertUtils.getAllFields(record)).anyMatch(field -> oConvertUtils.isNotEmpty(field.getAnnotation(Dict.class)));
    }
